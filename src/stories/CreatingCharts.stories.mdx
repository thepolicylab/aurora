...
The charts in the design system are React components, and to use them, we need to include them in `jsx` code. `jsx` is a JavaScript extension that allows HTML like syntax in JavaScript. React will compile `jsx` into plain JavaScript code. For more information on `jsx`, please see [this page](https://reactjs.org/docs/introducing-jsx.html).

## High-level design ideas

The design system is built to have APIs similar to other popular data visualization packages such as `seaborn`, `matplotlib`, and `ggplot`. The only difference is that the options are specified with `jsx`. Below is a standard bar chart:
...
Then we can adjust the data visualization, so we specify `title`, `subtitle`, `width` and `height`. For different chart types, there are other options that we can adjust. Please navigate to the documentation pages for these chart types. Be sure to check out code examples by clicking on "Show Code" on the bottom-right corner of each chart for code examples. Also feel free to adjust the options and see how they change how the charts look.